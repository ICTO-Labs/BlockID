        ...
        <v-expansion-panel v-for="validator in validators" :key="validator.id">
            <v-expansion-panel-title class="bg-containerBg">
                ...
                <template v-slot:actions>
                    <v-btn size="small" class="mr-2" @click.stop="$emit('open-dialog', { dialog: 'validator', method: 'edit', params: validator})">
                        <v-icon>mdi-pencil</v-icon>
                    </v-btn>
                    <v-btn size="small" color="error" @click.stop="deleteValidator(validator.id)">
            ...
            <v-expansion-panel-text>
                ...
                <CriteriaList :validatorId="validator.id" :criterias="validator.criterias" @show-params="$emit('show-params', $event)" @open-dialog="handleOpenDialog"  />
            </v-expansion-panel-text>
        </v-expansion-panel>
    </v-expansion-panels>
    <v-divider></v-divider>
    <div class="mt-4">
        <v-btn @click="$emit('open-dialog', {dialog: 'validator', method: 'add', params: {applicationId: props.applicationId}})">
