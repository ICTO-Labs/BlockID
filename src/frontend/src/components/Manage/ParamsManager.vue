        ...
        if (props.providerId !== null) {
            ...
            if (provider) {
                const _params = provider.params.map(param => {
                    ...
                    if(Array.isArray(props.providerParams) && props.providerParams.length > 0){
                        console.log('props.providerParams', props.providerParams);
                        const existingParam = props.providerParams.find(p => p.key === param.key);
                        return {
                            ...param,
            ...
            localParams.value = Array.isArray(props.providerParams) && props.providerParams.length > 0 ? props.providerParams[0] : [];
        }
    };

    // onMounted(initializeParams);

    watch(() => props.providerParams, initializeParams, { immediate: true, deep: true });
    ...
    <v-card>
        ...
        <v-card-text>
            <v-list-item v-for="(param, index) in localParams" :key="index">
                <v-row>
                        <v-col class="pa-1">
                            <v-text-field density="comfortable" v-model="param.key" label="Key" @blur="param.key && param.key.length > 0 ? param.key = param.key.trim() : param.key = ''" :disabled="isProvider" clearable @click:clear="removeParam(index)"></v-text-field>
                        </v-col>
                        <v-col class="pa-1">
                        ...
                        <v-col cols="auto">
                            <v-btn variant="text" @click="removeParam(index)" color="red"><v-icon>mdi-minus</v-icon></v-btn>
                        </v-col>
                    </v-row>
                </v-list-item>
            </v-card-text>
