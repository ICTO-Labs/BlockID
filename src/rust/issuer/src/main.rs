...
const ISSUER_URL: &str = "https://blockid.cc";
...
pub type WalletId = Principal;

#[derive(CandidType, Deserialize, Debug)]
struct WalletScoreResponse {
    pub linkedWallet: Option<(WalletId,candid::Nat,)>,
    pub linkedScore: candid::Nat,
    pub primaryScore: candid::Nat,
    pub totalScore: candid::Nat,
    pub percentileAbove: f64,
}
...
async fn prepare_credential(
    ...
    let credential_jwt = match prepare_credential_jwt(&req.credential_spec, &alias_tuple).await {
...
//Show simple config like derivation_origin and frontend_hostname
#[query]
#[candid_method(query)]
fn get_config_values() -> Result<(String, String), IssueCredentialError> {
    CONFIG.with_borrow(|config| {
        let config = config.get();
        Ok((
            config.derivation_origin.clone(),
            config.frontend_hostname.clone()
        ))
    })
}
pub fn get_vc_consent_message_en(
    ...
    match verify_credential_spec(credential_spec) {
        Ok(SupportedCredentialType::VerifiedScore(score)) => Ok(Icrc21ConsentInfo {
            consent_message: format!("You have BlockID score is {} or higher", score),
...
fn verify_verified_score_spec_and_get_score(spec: &CredentialSpec) -> Result<i32, String> {
    if spec.credential_type.as_str() == "VerifiedScore" {
        let Some(arguments) = &spec.arguments else {
            return Err("Credential spec has no arguments".to_string());
        };
        let expected_argument = "score";
        let Some(value) = arguments.get(expected_argument) else {
            return Err(format!(
                "Credential spec has no {}-argument",
                expected_argument
        ...
        let ArgumentValue::Int(score) = value else {
            return Err(format!(
                "Credential spec has unexpected value for {}-argument",
                expected_argument
            ));
...
fn verify_credential_spec(
    ...
    match spec.credential_type.as_str() {
        ...
        "VerifiedScore" => {
            let score = verify_verified_score_spec_and_get_score(spec)
                .map_err(IssueCredentialError::UnsupportedCredentialSpec)?;
            Ok(SupportedCredentialType::VerifiedScore(score))
        }
        "EventAttendance" => {
...
#[update]
#[candid_method]
async fn test_wallet_score(user: Principal) -> Result<String, String> {
    let total_score = get_wallet_score(user, "block-id".to_string())
        .await
        .map_err(|e| format!("Error getting wallet score: {}", e))?;
...
async fn get_wallet_score(principal_id: Principal, app_id: String) -> Result<candid::Nat, String> {
    let remote_canister_id = Principal::from_text("3c7yh-4aaaa-aaaap-qhria-cai")
        .map_err(|e| format!("Invalid remote canister ID: {}", e))?;

    let call_result: Result<(WalletScoreResponse,), _> = ic_cdk::api::call::call(
        remote_canister_id,
        "getWalletScore",
        (principal_id, app_id),
    ).await;
    ...
    match call_result {
        Ok((response,)) =>  Ok(response.totalScore),
        Err((code, msg)) => {
            Err(format!("Error calling remote canister {:?}: {}", code, msg))
        }
    }
}
